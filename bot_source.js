...
var SteamTotp = require('steam-totp'); //use npm install steam-totp@1.0.0 (steamcommunity has this dependancy)
...
var SteamTradeOffers = require('steam-tradeoffers'); //use npm install steam-tradeoffers
...
var admin = '76561198065442521'; //the id of the person that can use /commands and empty the bots inventory via chat etc

//contents of mafile
var TOTP_UserConfig =  ; //object containing shared_secret, identitiy_secret etc. get it by logging in the bot with this software https://github.com/Jessecar96/SteamDesktopAuthenticator/releases (after you log in you will have an mafile created, copy-paste the contents of that file here)
...
var minbet=0.03; //min value PER BET an user can deposit
...
var sitename = "needforskins.csgo2x.com"; //site url WITH NO http:// in front of it and NO forward slash ( / ) after it
...
client.on('webSession', function(sessionID, cookies) {
                ...
                offers.setup({
                    ...
                    console.log("Mobile Conf. set-up and set cookies for community");
});
    ...
    if(steamID.getSteamID64() == admin) {
        ...
        if (message.indexOf("/sendallitems") == 0) {
            ...
            offers.loadMyInventory({
                ...
                if (err) {
                    client.chatMessage(steamID, 'Error while trying to load invenotry to send items. Calling weblogon() - try again please.');
                ...
                if (num > 0) {
                    offers.makeOffer({
                        ...
                        if (err) {
                            client.chatMessage(steamID, 'Error while trying to send offer with items. Calling weblogon() - try again please.');
                            myconsolelog('Error while trying to send offer with items. Calling weblogon() - try again please.');
                            weblogon();
                            return;
                        }
                        client.chatMessage(steamID, 'Trade offer with all items from the inventory sent!');
                                            ...
                                            mysqlConnection.query('SELECT `totalvalue`,`itemsnum` FROM `games` WHERE `id`=\'' + current_game + '\'', function(err1, row1, fields1) {
                                              ...
                                              for (var j = 0; j < num; j++) {
                                                ...
                                                mysqlConnection.query('SELECT COUNT(DISTINCT userid) AS playerscount FROM `'+prf + current_game+'`', function(err, rows) {
                                                    ...
                                                    if (playerscount == 1 && items.length > 0 && gamestarted==='no') { //STARTING GAME
